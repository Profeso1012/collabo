...
export default function BlogComments({ blogId }: { blogId: string }) {
    ...
    <div key={comment.id} className={`${depth > 0 ? 'ml-8 mt-4' : 'mt-6'}`}>
      <div className="bg-gray-50 rounded-lg p-6">
        ...
        <button
          ...
          className="text-sm text-maroon hover:text-maroon-light font-medium"
        >
          Reply
        </button>
        ...
        {replyingTo === comment.id && (
          <div className="mt-4 p-4 bg-white rounded-lg border border-gray-200">
            <input
              ...
              className="w-full px-4 py-2 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-maroon mb-2"
            />
            <textarea
              value={replyContent}
            ...
            <div className="flex gap-2 mt-2">
              <button
                ...
                disabled={loading}
                className="bg-maroon text-white px-6 py-2 rounded-lg font-semibold hover:bg-maroon-light transition-all disabled:opacity-50"
              >
  ...
  return (
    <div>
      <h2 className="text-3xl font-bold text-maroon mb-8">Comments</h2>
      ...
      <form onSubmit={handleSubmit} className="mb-12">
        <input
          ...
          className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-maroon mb-3"
        />
        <textarea
          value={newComment}
        ...
        <button
          type="submit"
          disabled={loading}
          className="mt-4 bg-maroon text-white px-8 py-3 rounded-lg font-semibold hover:bg-maroon-light transition-all disabled:opacity-50"
        >
